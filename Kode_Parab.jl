...
# ╔═╡ 7983a970-dacc-11ef-090f-eb3090da1f67
using Plots,DifferentialEquations, PlutoUI,DataFrames

# ╔═╡ b6e7d15d-279e-41b0-9151-0180c495d1a1
md"## Persamaan Parabola"

# ╔═╡ c0369535-e7d4-42bc-948c-ed47b529e207
md"""###
!!! question "persamaan parabola yang digunakan"
	
	untuk sumbu x

	``\frac{dv_{0x}}{dt}=-\frac{b}{m} v_x``

	``\frac{dx}{dt}=v_x``

	dan untuk sumbu y

	``\frac{dv_{0y}}{dt}=-\frac{bv_y-mg}{m}``

	``\frac{dy}{dt}=v_y``

"""

# ╔═╡ 9f8d298e-38ee-42e0-a994-0e87462453cd
md" Dengan Kode nya sebagai berikut"

# ╔═╡ e850ce2a-e1ce-4858-90a6-db0e74284b67
function projectile_with_drag!(du, u, p, t)
	x, y, vx, vy = u    # u[1]=x, u[2]=y, u[3]=vx, u[4]=vy
	b, m, g = p         
	du[1] = vx            
	du[2] = vy            
	du[3] = -b/m * vx     
...
# ╔═╡ fb03801a-0c77-4fc2-9e77-debcc69a73bf
md"Untuk Initalisasi parameter sebagai berikut"

# ╔═╡ 0da2fe29-bcec-4f79-b043-6331917b80a9
md"nilai b = $@bind b Slider(0:0.01:1,show_value=true)"

# ╔═╡ a27df5c2-b9ef-454b-b9db-34ca5d37378e
md"nilai m = $@bind m Slider(1:0.1:100,show_value=true)"

# ╔═╡ bc061756-cbe6-4bf5-ba13-6d8847c11f0c
g = 9.8

# ╔═╡ 2658dbb7-eba2-4753-9eaf-fa735cd144d3
md"nilai ``v_0`` = $@bind v0 Slider(1:0.1:100,show_value=true)"

# ╔═╡ 2f2cf1cc-c150-4747-98f5-3f9a20ac7c1a
md"nilai ``\theta`` = $@bind θ Slider(0:0.01:π,show_value=true)"

# ╔═╡ e17931fa-4015-4b74-8ccd-0db8677dd82c
begin 
	x0, y0 = 0.0, 0.0       # Posisi awal
	vx0 = v0 * cos(θ)       # Komponen kecepatan horizontal
	vy0 = v0 * sin(θ)       # Komponen kecepatan vertikal
	u0 = [x0, y0, vx0, vy0]
	teval = (0.0, 10.0)
end

# ╔═╡ 44e8303b-f44b-45e8-9c0c-2f6d84aada35
begin 
	prob = ODEProblem(projectile_with_drag!, u0, teval, (b, m, g))
	...
	waktu = sol.t
	    ...
	    # Fungsi untuk lintasan analitis
    function Parabola_actually_plot(waktu)
        x = (m / b) * vx0 * (1 .- exp.(-b .* waktu / m))
        y = (m / b) * (m * g / b + vy0) * (1 .- exp.(-b .* waktu / m)) - (m * g *
		waktu / b)
        return x, y
    end

    # Lintasan analitis
    xact, yact = Parabola_actually_plot(waktu)

	
	plot(x_t, y_t, 
	     xlabel="Posisi Horizontal (x)", 
	     ylabel="Posisi Vertikal (y)", 
	     label="Hasil Numerik", 
	     title="Gerak Parabola dengan Gesekan", 
	     lw=2, color=:blue)
	plot!(xact, yact,  
	     lw=2,label="Hasil Analitik", color=:red)
end

# ╔═╡ 29153833-9da2-47d6-8be6-f7d46d5d1a3b
begin 
	# Menghitung error/
    error_x = abs.(x_t .- xact)
    error_y = abs.(y_t .- yact)
	Error_Tabel = df = DataFrame(ErrorX = error_x, ErrorY = error_y)
end

# ╔═╡ 36d66e4f-50b4-445f-9bc6-4cd4c1547170
begin 
	MeanX = sum(error_x) / length(error_x)
	MeanY = sum(error_y) / length(error_y)
	stdx = sqrt(sum((error_x .- MeanX).^2) / (length(error_x) - 1))
	stdy = sqrt(sum((error_y .- MeanY).^2) / (length(error_y) - 1))

	Mean_std_tabel = DataFrame(MeanX = MeanX, Mean_Y=MeanY,Std_x=stdx,Std_y=stdy)
end

# ╔═╡ 8356039f-754c-4878-bbfe-8be5a82e9370
begin 
    vx = sol[3,:] 
    vy = sol[4,:] 
	
	p1 = plot(waktu,vx, label="vx",color=:blue)
	p2 = plot(waktu,vy, label="vy",color=:red)
...
manifest_format = "2.0"
project_hash = "11360d39f6a018950191781fb522168aa681dbb0"
